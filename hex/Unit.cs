...
/*                if(moveCost == 555555)
    ...
    public bool TryMoveToGameHex(GameHex targetGameHex, TeamManager teamManager)
    {
        ...
        if(moveCost <= remainingMovement)
        {
            ...
            else if(!targetGameHex.units.Any() && (targetGameHex.district.health == 0 && !Global.gameManager.game.teamManager.GetAllies(teamNum).Contains(Global.gameManager.game.cityDictionary[targetGameHex.district.cityID].teamNum)))
            {
                SetRemainingMovement(remainingMovement - moveCost);
                Global.gameManager.game.mainGameBoard.gameHexDict[hex].units.Remove(this.id);
                Hex previousHex = hex;
